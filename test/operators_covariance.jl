...
@testset "SpectralCovariance - FourierTransform" begin
    ...
    z = rand(3, M, M, M, 12)
    μ, D, V = sc(z)
    @test size(μ) == (1, M, M, M, 12)
    @test size(D) == (1, M, M, M, 12)
    @test size(V) == (1, M, M, M, rank, 12)
    @test all(D .> 0)

    trafo = FourierTransform((4, 6), (3, 4))
    ch = 3 => 2
    rank = 1
    sc = SpectralCovariance(trafo, rank, ch)
    M = 32
    z = rand(3, 16, M, M, 12)
    μ, D, V = sc(z)
    @test size(μ) == (2, 16, M, M, 12)
    @test size(D) == (2, 16, M, M, 12)
    @test size(V) == (2, 16, M, M, rank, 12)
    @test all(D .> 0)

    trafo = FourierTransform((4,), (3,))
    ch = 3 => 2
    rank = 1
    sc = SpectralCovariance(trafo, rank, ch)
    M = 32
...
@testset "SpectralCovariance - ChebyshevTransform" begin
    ...
    trafo = ChebyshevTransform((4, 6), (3, 4))
    ch = 3 => 2
    rank = 1
    sc = SpectralCovariance(trafo, rank, ch)
    M = 32
