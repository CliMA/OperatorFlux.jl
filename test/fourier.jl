...
@testset "FourierTransform" begin
    ...
    @test norm(OperatorFlux.inverse(trafo, forward(trafo, z, 2:2), 2:2) - z) ≤ 2 * eps(norm(z))
    @test norm(OperatorFlux.inverse(trafo, forward(trafo, z, (3,)), (3,)) - z) ≤ 2 * eps(norm(z))
    ...
    c = rand(27, 16, 14, 1)
    inds = [[(1:div(m, 2)+1)..., (s-div(m, 2)+2:s)...] for (s, m) in zip((M, 14), size(c)[2:end-1])]
...
@testset "FourierTransform - ChainRulesCore" begin
    ...
    test_rrule(OperatorFlux.truncate_modes, trafo, c, dims, check_thunked_output_tangent = false)
